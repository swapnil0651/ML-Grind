...
# Add the %CPU used by each process to the labels
labels_with_usage = [f"{user} ({cpu:.1f}%)" for user, cpu in zip(CPU_usage_by_user_sorted['USER'], CPU_usage_by_user_sorted['%CPU'])]
labels_with_usage.append(f"Unused CPU ({unused_CPU:.1f}%)")  # Add unused CPU
...
# Use a legend to show user and %CPU used by each process
plt.legend(patches, labels_with_usage, loc="best", bbox_to_anchor=(1, 0.5))
...
# Adjust the layout to make everything fit
plt.tight_layout()
